...
- Supabase backend. `supabase-setup.sql` creates:
  ...
  - `team_pins` (hashed PINs, not readable)
  - `submit_ballot()` RPC, the only write path. It checks or sets the team PIN and validates the ballot.
  - Realtime on `ballots`
- Sleeper API (no auth) for teams, records, avatars, and the current week. League ID: 1312128506452283392
- Hosting: Netlify, deployed from GitHub on push. Custom domain wcxcdynasty.site.
- `netlify.toml` sets the security headers, including a CSP that allowlists every host the page talks to. Adding a CDN, font host, or second Supabase project means updating it or the browser blocks the request silently.
