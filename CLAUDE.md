...
- `ktc.json` — dynasty superflex values keyed by Sleeper player id, plus Mid-tier pick values per season/round. Built by `node tools/refresh-ktc.js`. KTC has no API and no CORS, so the name→id mapping is resolved at build time. Team value = players + future picks; pick ownership resolves live from Sleeper `traded_picks`, skipping seasons whose draft already happened.
